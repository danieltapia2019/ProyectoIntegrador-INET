    ...
    <div class="contenido col-md-10">
        ...
        <ul class="pagination nav-link">
            ...
            <li class="@if($currentPage==$firstPage){{'disabled'}}@endif">
                <a href="@if($currentPage>1){{$consultas->url($firstPage).$link}}@else{{$consultas->url($firstPage).$link}}@endif"
                    class='btn'>Primera</a>
            </li>
            <!-- Botón para navegar a la página anterior -->
            <li class="@if($currentPage==$firstPage){{'disabled'}}@endif">
                <a href="@if($currentPage>1){{$consultas->url($forwardPage).$link}}@else{{$consultas->url($firstPage).$link}}@endif"
                    class='btn'>«</a>
            </li>
            ...
            @for($x=$currentPage;$x<$maxPages;$x++) @if($x <=$lastPage) <li
                ...
                <li class="@if($currentPage==$lastPage){{'disabled'}}@endif">
                    <a href="@if($currentPage<$lastPage){{$consultas->url($nextPage).$link}}@else{{'#'}}@endif"
                        class='btn'>»</a>
                </li>
                <!-- Botón para navegar a la última página -->
                <li class="@if($currentPage==$lastPage){{'disabled'}}@endif">
                    <a href="@if($currentPage<$lastPage){{$consultas->url($lastPage).$link}}@else{{'#'}}@endif"
                        class='btn'>Última</a>
                </li>
        </ul>
    </div>
